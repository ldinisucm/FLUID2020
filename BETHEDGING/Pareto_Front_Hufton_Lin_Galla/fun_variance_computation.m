function [Var_theor,lambda_theor]=fun_variance_computation(kappa_pi_set,ks,numpuntos_ave,numpuntos_var)
...
%x=linspace(eps,(phi1plus-phi2plus)^g,numpuntos_ave);
Dx=x(2)-x(1)
...
%x=linspace(eps,(phi1plus-phi2plus)^h,numpuntos_ave);
Dx=x(2)-x(1);
...
%x=linspace(0+eps,x_bar-eps,numpuntos_ave);
Dx=x(2)-x(1);
%g
%min(phi1plus-phi1minus-x.^(1/g))
%min(phi2minus+x.^(1/g)-phi1plus)
