...
%Added in 2023 september
%Computes extinction probabilities
...
odds=[2.3 1.9 ]';
odds=[2.3 2 ]';
r=[0.7 0.1 0.2]'; %trade-off1 values
r
%r=[0.11 0.24 0.65]';
sum(r)
V=-log(sum(r))
...
%b=r+[0 0 .5];
b=P+8*[0.2 -.15 .05];
b= [0.5594    0.1388    0.3018]'
b=[0.6565    0.1139    0.2296]'
norm_b=sum(b)
b=b/norm_b
b=b(:,1).*ones(size(P))
...
for k=1:num_umbrales
	...
	for i=2:num_races+1
	     rn=rand; %random number from 0 to 1;
	     ...
	     check=(rn<=cumulative_p); %check is a vector with elements 0 or 1 depending whether the condition is met or not. The first appearance of 1 is the horse that wins
