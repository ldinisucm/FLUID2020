...
int_tau=Dtau*sum(tau.^(-x).*real(j*w*tau./(1+j*w*tau.*(1+G0./(G./w^2)))));
%int_tau=Dtau*sum(tau.^(-x).*(w^2*tau.^2*(1+G0/G)./(1+w^2*tau.^2*(1+G0/G)^2)));
gpp(i)=G0*Dtau*sum(tau.^(-x).*imag(j*w*tau./(1+j*w*tau.*(1+G0./(G./w^2)))))+eta0*(w);
